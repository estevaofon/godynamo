...
import (
	...
	"github.com/godynamo/internal/ui/textarea/memoization"
...
func DefaultStyles() (Style, Style) {
	focused := Style{
		...
		Selection:        lipgloss.NewStyle().Background(lipgloss.Color("212")).Foreground(lipgloss.Color("0")),
	}
	blurred := Style{
		Base:             lipgloss.NewStyle(),
		...
		Selection:        lipgloss.NewStyle().Background(lipgloss.Color("212")).Foreground(lipgloss.Color("0")), // Pink background, Black text
	}
...
func (m Model) View() string {
	...
	for l, line := range m.value {
		...
		for wl, wrappedLine := range wrappedLines {
			...
			// RENDER CONTENT WITH SELECTION

			// Range for this wrapped line in logical columns:
			// [runningCol, runningCol + len(wrappedLine))
			wlStart := runningCol
			wlEnd := runningCol + len(wrappedLine)

			// Simple check if this line intersects selection
			intersects := false
			if m.selectionActive {
				// Check row coverage
				if l > m.selStartRow && l < m.selEndRow {
					intersects = true // Fully selected row
				} else if l == m.selStartRow && l == m.selEndRow {
					// Single line selection
					// Intersects if [wlStart, wlEnd) overlaps [selStart, selEnd)
					if wlStart < m.selEndCol && wlEnd > m.selStartCol {
						intersects = true
					}
				} else if l == m.selStartRow {
					// Start row
					if wlEnd > m.selStartCol {
						intersects = true
					}
				} else if l == m.selEndRow {
					// End row
					if wlStart < m.selEndCol {
						intersects = true
					}
				}
			}
			...
			if !intersects {
				...
				// Slow path: split by selection
				// We iterate runes to apply styles
				for i, r := range wrappedLine {
					...
					if l > m.selStartRow && l < m.selEndRow {
						...
						if absCol >= m.selStartCol && absCol < m.selEndCol { // Exclusive end
							selected = true
						}
					} else if l == m.selStartRow {
						if absCol >= m.selStartCol {
							selected = true
						}
					} else if l == m.selEndRow {
						if absCol < m.selEndCol {
							selected = true
						}
					}
					...
					charStyle := style
					if selected {
						charStyle = m.style.Selection
					}

					// Cursor handling overrides selection? No, cursor is usually inverse or block.
					// If cursor is here:
					if m.row == l && lineInfo.RowOffset == wl && i == lineInfo.ColumnOffset {
						m.Cursor.SetChar(string(r))
						// Cursor view usually has its own style.
						s.WriteString(m.Cursor.View())
					} else {
						s.WriteString(charStyle.Render(string(r)))
					}
				}
				// Handle cursor at VERY END of line (outside wrappedLine range)
				if m.row == l && lineInfo.RowOffset == wl && m.col >= len(line) && lineInfo.CharOffset >= m.width {
					m.Cursor.SetChar(" ")
					s.WriteString(m.Cursor.View())
				}
			}
...
// LogicalCursor returns the logical row and column of the cursor.
// These correspond to the index in the value slice []line.
func (m Model) LogicalCursor() (int, int) {
	return m.row, m.col
}
...
// It normalizes the range so start is always before end.
func (m *Model) SetSelection(startRow, startCol, endRow, endCol int) {
	m.selectionActive = true
	// Normalize
	if startRow > endRow || (startRow == endRow && startCol > endCol) {
		m.selStartRow, m.selStartCol = endRow, endCol
		m.selEndRow, m.selEndCol = startRow, startCol
	} else {
...
// IsSelected checks if a specific position is within the selection.
func (m *Model) IsSelected(row, col int) bool {
	if !m.selectionActive {
		return false
	}
	if row < m.selStartRow || row > m.selEndRow {
		return false
	}
	if row == m.selStartRow && col < m.selStartCol {
		return false
	}
	if row == m.selEndRow && col >= m.selEndCol { // selection is exclusive at end col usually? No, inclusive for vim?
		// Vim visual selection is inclusive of the character under cursor.
		// My extractText logic was exclusive?
		// extractText: sCol:eCol (exclusive).
